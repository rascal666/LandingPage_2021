...
class Ondata {
    constructor(dom) {
        ...
        if (_dom) {
            ...
            if (this.utm.cl) {
                const _event = document.querySelectorAll('._event');
                _event.forEach(element => {
                    element.innerHTML = element.innerHTML.replace('НК_', this.generalClass[this.utm.cl]);
                });
            }
            ...
            if (this.generalNumbers[this.utm.utm_rascal]) {
                ...
                if (this.utm.utm_rascal == 'by' && _contact__main) {
...
let ree = "позволяющий моментально пополнить линейку товаров от лидеров производства."
// пользуешься созданным классом
onData.setInnerText("kz", /Вся Россия/gi, "Весь Казахстан");
onData.setInnerText("ua", /Вся Россия/gi, "Вся Украина");

onData.setInnerText("kz", /со всей страны/gi, "со всего Казахстана");
onData.setInnerText("ua", /со всей страны/gi, "со всей Украины");

onData.setInnerText("by", /Вы подскажете шефу, что с тарифом “Вся Россия” на Пульсе цен можно получать клиентов со всей страны, не выходя из кабинета./gi, "Вы подскажете шефу, что на Пульсе цен есть возможность размещать товары в любом городе страны, и осуществлять продажи не выходя из кабинета.");

onData.setInnerText("by", ree, "позволяющий загрузить на сайт 100 000 товаров. ");
onData.setInnerText("by", /поможет новый проект Пульса цен/gi, "поможет тариф на Пульсе цен");
onData.setInnerText("by", /Дилер-Дистрибьютор/gi, "Платина+");
